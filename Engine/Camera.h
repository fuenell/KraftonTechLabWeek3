...
public:
    UCamera()
        ...
        , mPitch(0.0f)
    {
        // 초기 방향: 로컬 Forward(+Y)가 월드의 -Y로 가도록(=아이덴티티면 Forward는 -Y)
        RecalcAxesFromQuat();
        UpdateView();
        UpdateProj();
    }
    ...
    {
        mEye = eye;
        FVector f = (target - eye).GetNormalized(); // 카메라가 바라볼 +forward
        mRot = FQuaternion::LookRotation(f, up);
        mRot = mRot.Normalized();
        RecalcAxesFromQuat();

        // pitch 추정(옵션): forward.z = sin(pitch)
        float fz = mForward.Z;
        fz = (fz < -1.f ? -1.f : (fz > 1.f ? 1.f : fz));
        mPitch = asinf(fz);
    ...
    // 회전: yawZ는 "세계 Z축" 기준, pitch는 "카메라 Right(현재)" 기준
...
private:
    ...
    void RecalcAxesFromQuat()
    {
        ...
        mForward = mRot.Rotate(FVector(0, -1, 0)).GetNormalized(); // -Y (규약)
    }
