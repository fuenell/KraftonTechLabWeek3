...
namespace json {
    ...
    public:
        ...
        public:
            inline JSONWrapper(Container* val) : object(val) {}
            inline JSONWrapper(std::nullptr_t) : object(nullptr) {}

            inline typename Container::iterator begin() { return object ? object->begin() : typename Container::iterator(); }
            inline typename Container::iterator end() { return object ? object->end() : typename Container::iterator(); }
            inline typename Container::const_iterator begin() const { return object ? object->begin() : typename Container::iterator(); }
            inline typename Container::const_iterator end() const { return object ? object->end() : typename Container::iterator(); }
        };
        ...
        public:
            inline JSONConstWrapper(const Container* val) : object(val) {}
            inline JSONConstWrapper(std::nullptr_t) : object(nullptr) {}

            inline typename Container::const_iterator begin() const { return object ? object->begin() : typename Container::const_iterator(); }
            inline typename Container::const_iterator end() const { return object ? object->end() : typename Container::const_iterator(); }
        };
        ...
        template <typename T>
        inline JSON(T b, typename enable_if<is_same<T, bool>::value>::type* = 0) : Internal(b), Type(Class::Boolean) {}

        template <typename T>
        inline JSON(T i, typename enable_if<is_integral<T>::value && !is_same<T, bool>::value>::type* = 0) : Internal((long)i), Type(Class::Integral) {}

        template <typename T>
        inline JSON(T f, typename enable_if<is_floating_point<T>::value>::type* = 0) : Internal((double)f), Type(Class::Floating) {}

        template <typename T>
        inline JSON(T s, typename enable_if<is_convertible<T, string>::value>::type* = 0) : Internal(string(s)), Type(Class::String) {}

        inline JSON(std::nullptr_t) : Internal(), Type(Class::Null) {}
        ...
        template <typename T>
        inline typename enable_if<is_same<T, bool>::value, JSON&>::type operator=(T b) {
            SetType(Class::Boolean); Internal.Bool = b; return *this;
        }

        template <typename T>
        inline typename enable_if<is_integral<T>::value && !is_same<T, bool>::value, JSON&>::type operator=(T i) {
            SetType(Class::Integral); Internal.Int = i; return *this;
        }

        template <typename T>
        inline typename enable_if<is_floating_point<T>::value, JSON&>::type operator=(T f) {
            SetType(Class::Floating); Internal.Float = f; return *this;
        }

        template <typename T>
        inline typename enable_if<is_convertible<T, string>::value, JSON&>::type operator=(T s) {
        ...
        inline JSON& at(const string& key) {
            return operator[](key);
        }

        inline const JSON& at(const string& key) const {
            return Internal.Map->at(key);
        }

        inline JSON& at(unsigned index) {
    ...
    private:
        inline void SetType(Class type) {
            if (type == Type)
                return;
